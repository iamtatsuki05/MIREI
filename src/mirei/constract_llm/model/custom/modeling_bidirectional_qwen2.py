...
class Qwen2BiModel(Qwen2Model, SentenceEncoderMixin):
    ...
    # Copied verbatim from modeling_bidirectional_llama.py (the LLM2Vec-style bidirectional adaptation
    # of the upstream causal-mask helper: the commented-out blocks are the upstream causal code).
    # transformers 4.56.1 no longer calls this hook from Qwen2Model.forward, so the forward() override
    # below calls it explicitly.
    ...
    # Copied verbatim from modeling_bidirectional_llama.py
    @staticmethod
    ...
    # Copied verbatim from transformers 4.56.1 Qwen2Model.forward, except for the marked block that
    # replaces the upstream causal mask creation with the bidirectional _update_causal_mask above.
    # The upstream @auto_docstring decorator is dropped: it raises an IndexError for classes defined
    # outside the transformers package.
    @check_model_inputs
    def forward(
        self,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            # === MODIFIED from transformers 4.56.1 (bidirectional attention) ===
            # The upstream code below built causal masks; the bidirectional mask is built instead.
            ...
            bidirectional_mask = self._update_causal_mask(
                attention_mask,
                inputs_embeds,
                cache_position,
                past_key_values,
            )
            causal_mask_mapping = {'full_attention': bidirectional_mask}
            # A bidirectional encoder attends over the full sequence on sliding layers as well.
            if self.has_sliding_layers:
                causal_mask_mapping['sliding_attention'] = bidirectional_mask
            # === END MODIFIED ===
...
class Qwen2BiForSequenceClassification(Qwen2PreTrainedModel):
    def __init__(
        self,
        config: Qwen2Config,
        pooling_mode: str = 'mean',
