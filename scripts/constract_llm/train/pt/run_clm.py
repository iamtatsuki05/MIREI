...
from nlp.constract_llm.train.language_model.clm.data_class.data_training_arguments import (
    DataTrainingArguments,
)
from nlp.constract_llm.train.language_model.clm.data_class.model_arguments import (
    ModelArguments,
)
...
def main(config_file_path: str | Path, **kwargs: Any) -> None:
    ...
    column_names = (
        list(raw_datasets['train'].features) if training_args.do_train else list(raw_datasets['validation'].features)
    )
    ...
    def tokenize_function(examples):
        ...
        # clm input could be much much longer than max_seq_length
    ...
    if data_args.max_seq_length is None:
        ...
        if max_seq_length > max_pos_embeddings:
            logger.warning(
                f'The tokenizer picked seems to have a very large `model_max_length` ({tokenizer.model_max_length}). '
                f'Using max_seq_length={min(1024, max_pos_embeddings)} instead. You can change that default value by passing --max_seq_length xxx.'
            )
            if max_pos_embeddings > 0:
                max_seq_length = min(1024, max_pos_embeddings)
            else:
                max_seq_length = 1024
    else:
