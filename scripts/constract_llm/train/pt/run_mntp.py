...
def main(config_file_path: str | Path | None = None, **kwargs) -> None:
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, CustomArguments))
    model_args, data_args, training_args, custom_args = parser.parse_dict(load_cli_config(config_file_path, **kwargs))
    ...
    # We’re using AutoModelForCausalLM.
    # Please add the following to your config.json:
    # {
    #   "auto_map": {
    #     "AutoModelForCausalLM": "MNTPModel"
    #   }
    torch_dtype = (
    ...
    # Custom: model organization is MODEL_TYPEBiForMNTP.model -> MODEL_TYPELBiModel, we have to apply PEFT to the inner model
    if custom_args.lora:
